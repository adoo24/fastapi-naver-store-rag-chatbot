...
# SSE 스트리밍 방식으로 GPT 응답을 생성
async def stream_answer_question(session_id: str, question: str):
    """
    SSE 스트리밍 방식으로 GPT 응답을 생성하며, 0.1초 딜레이를 추가.
    """
    if not session_id:
        session_id = str(uuid.uuid4())  # 고유한 session_id 생성
    ...
    related_questions = search_faq(refined_question)
    faq_context = "\n\n".join([f"- {q}" for q in related_questions])
    # yield f"data: 유사 질문 목록: {faq_context}\n\n"
    ...
    # 응답을 문장 단위로 분리하여 스트리밍
    for sentence in bot_response.split(". "):  # 문장 단위로 분리
